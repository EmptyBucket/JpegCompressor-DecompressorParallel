...
{
    public class YCbCrChannelsPacker : IChannelsPacker<YCbCrChannels>
    {
        public YCbCrPixel[,] Pack(YCbCrChannels yCbCrChannels, int height, int width)
        {
           ...
           var yCbCrPixels = new YCbCrPixel[sizeY, sizeX];
            for (var i = 0; i < sizeY; i++)
                for (var j = 0; j < sizeX; j++)
                    yCbCrPixels[i, j] = new YCbCrPixel(yCbCrChannels.YChannel[i, j], yCbCrChannels.CbChannel[i, j],
